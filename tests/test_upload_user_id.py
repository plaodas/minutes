...
def test_upload_sets_user_id(monkeypatch):
    ...
    if hasattr(tasks.process_audio, "delay"):
        monkeypatch.setattr(tasks.process_audio, "delay", lambda path: DummyTask("fake-user-id-1234"))
    else:
        monkeypatch.setattr(tasks, "process_audio", lambda path: DummyTask("fake-user-id-1234"))
