...
test.describe('modal focus trap', () => {
  test.beforeEach(async ({ page }) => {
          ...
          histories: [
            {
              event_ts: now,
              event_type: 'status',
              payload: { result: { output_file: 'outputs/1.txt' } },
            },
            { event_ts: now, event_type: 'progress', payload: {} },
            {
              event_ts: now,
              event_type: 'success',
              payload: { result: { output_file: 'outputs/2.txt' } },
            },
          ],
        },
      ];
    ...
    await page.route('**/bg/tasks**', async (route) => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({ tasks: sample }),
      });
    });
    ...
    await page.waitForSelector('[data-testid="view-history-sample-1"]', {
      state: 'attached',
      timeout: 10000,
    });
  ...
  test('renames the task and updates the modal title', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: 'Full history for Renamed task' })
    ).toBeVisible();
  });
