...
test('modal load more appends histories', async ({ page }) => {
  ...
  await page.evaluate(() => {
    document.querySelector('button[aria-label="History"]')?.click();
  });
  await page.waitForSelector('text=Recent minutes');
  const hasHistoryBtn = await page.evaluate(
    () => !!document.querySelector('button[aria-label="History"]')
  );
  console.log('HAS_HISTORY_BTN', hasHistoryBtn);
  const labels = await page.evaluate(() =>
    Array.from(document.querySelectorAll('button[aria-label]')).map((b) =>
  ...
  const count = await page.evaluate(
    () => document.querySelectorAll('[data-testid^="view-history-"]').length
  );
