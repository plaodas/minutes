...
test('offline fallback uses cached tasks and shows retry toast', async ({ page }) => {
  ...
  await page.addInitScript(() => {
    ...
    try {
      localStorage.setItem('recent_tasks', JSON.stringify(cached));
    } catch (e) {}
  });
    ...
    window.fetch = (input: any, init?: any) => {
      ...
      if (url.includes('/bg/minutes/')) {
        return Promise.resolve(
          new Response('cached minutes text', {
            status: 200,
  ...
  const fetchResult = await page.evaluate(
    ...
    process.env.E2E_PORT
      ? `http://localhost:${process.env.E2E_PORT}/bg/tasks`
  ...
  const testIds = await page.evaluate(() =>
    Array.from(document.querySelectorAll('[data-testid^="view-minutes-"]')).map((el) =>
  ...
  await page.waitForFunction(
    () => {
      ...
      return (
        text.includes('履歴の読み込みに失敗しました') ||
        text.includes('履歴を読み込めませんでした') ||
        text.includes('最新の履歴を取得できませんでした') ||
        text.includes('Could not retrieve the latest history') ||
        text.includes('Displaying older data')
      );
    },
    { timeout: 15000 }
  );
