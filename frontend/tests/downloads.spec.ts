...
test('download buttons call backend endpoints', async ({ page }) => {
  ...
  await page.addInitScript(() => {
    ...
    try {
      localStorage.setItem('recent_tasks', JSON.stringify(sample));
    } catch (e) {}
  });
    ...
    window.fetch = (input: any, init?: any) => {
      ...
      if (url.includes('/bg/tasks')) {
        ...
        return Promise.resolve(
          new Response(JSON.stringify({ tasks: sample }), {
            status: 200,
      ...
      if (url.includes('/bg/minutes/')) {
        return Promise.resolve(
          new Response('Sample minutes for download test', {
            status: 200,
  ...
  await page.addInitScript(() => {
    ...
    window.fetch = (input: any, init?: any) => {
      ...
      if (url.includes('/api/bg/transcript/')) {
        ...
        return Promise.resolve(
          new Response('TRANSCRIPT', { status: 200, headers: { 'Content-Type': 'text/plain' } })
        );
      }
      ...
      if (url.includes('/api/bg/action-items/')) {
        ...
        return Promise.resolve(
          new Response(JSON.stringify({ items: [{ text: 'Do thing' }] }), {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
          })
        );
  ...
  await page.goto(
    process.env.E2E_PORT ? `http://localhost:${process.env.E2E_PORT}` : 'http://localhost:8080'
  );
  ...
  await page.waitForSelector('button[aria-label^="View minutes for"]', {
    state: 'attached',
    timeout: 10000,
  });
  ...
  await transcriptBtn.evaluate((el: HTMLElement) => {
    el.scrollIntoView();
    el.click();
  });
  ...
  await summaryBtn.evaluate((el: HTMLElement) => {
    el.scrollIntoView();
    el.click();
  });
  ...
  await actionsBtn.evaluate((el: HTMLElement) => {
    el.scrollIntoView();
    el.click();
  });
  ...
  const counts = await page.evaluate(() => ({
    t: (window as any).__transcriptCalled || 0,
    s: (window as any).__summaryCalled || 0,
    a: (window as any).__actionsCalled || 0,
  }));
