    ...
    <>
      <section>
        ...
        <div ref={listRef} className="overflow-hidden rounded-lg border border-slate-200 bg-white">
          ...
          {items.map((item) => (
            <div key={item.id} data-testid={`view-minutes-${item.id}`} role="button" tabIndex={0} aria-labelledby={`history-title-${item.id}`} aria-describedby={`history-desc-${item.id}`} onClick={() => { setMinutesTask(item.id); setIsMinutesVisible(true) }} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setMinutesTask(item.id); setIsMinutesVisible(true) } }} className="cursor-pointer flex w-full items-center gap-3 border-b border-slate-100 p-4 text-left last:border-0 hover:bg-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[var(--accent)] focus-visible:shadow-lg focus-visible:bg-slate-50 transition-shadow">
