...
export function HistoryView({ onCreate }: { onCreate: () => void }) {
  ...
  useEffect(() => {
    const sentinel = document.querySelector(
      '[data-testid="history-list-sentinel"]'
    ) as HTMLElement | null;
    if (!sentinel) return;
  ...
  return (
    <>
      <section>
        ...
        {tasksError && (
          <div
            className={`mb-4 rounded-md p-3 border ${items && items.length > 0 ? 'bg-yellow-50 border-yellow-200' : 'bg-rose-50 border-rose-200'}`}
          >
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="shrink-0">
                <button
                  onClick={() => reload()}
                  className={`rounded px-3 py-1 text-sm font-medium text-white ${items && items.length > 0 ? 'bg-yellow-600 hover:brightness-90' : 'bg-rose-600 hover:brightness-90'}`}
                >
                  Retry
                </button>
              </div>
      ...
      {modalTask && (
        <div
          className={`fixed inset-0 z-50 flex items-start justify-center p-6 transition-opacity duration-200 ${isModalVisible ? 'bg-black/40 opacity-100' : 'bg-black/0 opacity-0'}`}
        >
          <button
            type="button"
          ...
          <div
            role="dialog"
            aria-modal="true"
            aria-label={`Full history for ${modalTask}`}
            className={`relative z-10 max-h-[80vh] w-full max-w-2xl overflow-auto rounded bg-white p-6 transform transition-all duration-200 ${isModalVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3'}`}
          >
            <button
              aria-label="Close"
              onClick={() => closeModal()}
            ...
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-semibold">
                Full history for {items.find((it) => it.id === modalTask)?.name || modalTask}
              </h2>
              <div>
                <button
                  data-testid={`rename-${modalTask}`}
                  onClick={async () => {
                    const newName = window.prompt(
                      'Enter new name for this task',
                      items.find((it) => it.id === modalTask)?.name || ''
                    );
                    if (!newName) return;
                    try {
                      await renameBgTask(modalTask, newName);
                      setItems((prev) =>
                        prev.map((it) => (it.id === modalTask ? { ...it, name: newName } : it))
                      );
                    } catch (e) {
                      // ignore
                    }
    ...
    const onAuth = () => {
      import('../api/client').then(({ getUserFeatures }) => {
        getUserFeatures()
          ...
          .catch(() => setFeatures({ authenticated: false }));
      });
    };
