  ...
  return (
    <div className="fixed bottom-6 right-6 flex flex-col gap-2 z-50">
      {toasts.map((t) => (
        <div
          key={t.id}
          className={`p-3 rounded shadow-lg max-w-xs ${t.type === 'error' ? 'bg-red-50 border border-red-200' : 'bg-white'}`}
        >
          <div className={`text-sm ${t.type === 'error' ? 'text-red-700' : 'text-gray-900'}`}>
            {t.message}
          </div>
        </div>
      ))}
