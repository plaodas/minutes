...
describe('fetchWithRetry', () => {
  ...
  it('does not retry on 4xx', async () => {
    globalThis.fetch = vi
      .fn()
      .mockImplementation(() => Promise.resolve(new Response('not found', { status: 404 }))) as any;
    await expect(
      fetchWithRetry('http://example.test/404', {}, { retries: 2, timeoutMs: 500 })
    ).rejects.toBeDefined();
  ...
  it('aborts on timeout', async () => {
    ...
    await expect(
      fetchWithRetry('http://example.test/slow', {}, { retries: 0, timeoutMs: 50 })
    ).rejects.toBeDefined();
  });
});
